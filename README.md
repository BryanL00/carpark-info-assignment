...
**Tech stack:** .NET Core 6.x, SQLite, Entity Framework Core 6, CsvHelper, Swashbuckle (Swagger)

**Architecture:** Clean layered architecture — Domain → Infrastructure → Api. Repository pattern with interfaces to keep layers decoupled and testable.
...
Open [http://localhost:5242/swagger](http://localhost:5242/swagger) to explore and test all endpoints interactively.

### Import Data

In Swagger, use `POST /api/batch/import` to upload the CSV file (`hdb-carpark-information-20220824010400.csv`). This seeds the database. The entire file rolls back if any row fails to process.
...
16 tests across 3 test classes — import service (integration tests with real SQLite), and controller tests (unit tests with Moq).
